...
6. **Preflight the login check.** Before spawning anything, detect that the user is not authenticated and exit with a single clear message telling them to run `claude`, log in, then re-run the task. This must be distinguishable from the existing "claude not on PATH" failure — a logged-out user and a missing install must not produce the same message.
   ...
   **6a. Use the exit code. Do not parse JSON.** The preflight needs exactly one bit — authenticated or not — and the exit code already carries it. Parsing `claude auth status --json` couples this launcher to an undocumented output shape for zero gain, and any "defensive parsing with a fallback" design degrades to the exit code anyway, so it is strictly more code to reach the same answer with more surface to get wrong. Treat the JSON shape as out of bounds; do not guess field names.
...
- Req 6a: no JSON parsing anywhere in the auth preflight. Grep for `--json`, `JSON.parse`, and any field-name guesses (`loggedIn`, `authenticated`, `status`) on the auth path — all must be absent. The decision reads an exit code only.
