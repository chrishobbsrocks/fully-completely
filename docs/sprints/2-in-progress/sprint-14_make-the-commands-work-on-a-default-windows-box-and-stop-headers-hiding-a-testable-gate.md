...
1. **No command file writes to a hardcoded POSIX path.** *Three remain, not five — `sprint-qa1.md` and `sprint-liveqa.md` were fixed in sprint 12 and shipped in 0.1.13, using working-directory-relative paths (`> qa1-notes-<N>.txt`). **That is a proven, shipped pattern; follow it rather than inventing a second one.*** Still outstanding: **`sprint-abort.md`** (`--reason-file`), **`sprint-complete.md`** (`--user-said-file`), and **`sprint-new.md`** (`--title-file` and `--epic-file`).
   - **`sprint-new.md` carries a second defect on the same lines.** It instructs writing the temp file *with the Write tool* — which sprint 12's scoped headless profile hard-disables, so a headless Master Controller cannot create a sprint at all. QA1 caught this class in sprint 12: sprint 11's Req 7 mandated a Write-tool pattern that sprint 12's own profile then made impossible. Fix both together or the file is still broken headless.
