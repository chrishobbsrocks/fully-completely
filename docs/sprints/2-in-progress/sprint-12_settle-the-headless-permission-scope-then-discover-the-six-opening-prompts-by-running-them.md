...
- **Two criteria sprint 11 could not demonstrate, carried here because this sprint is what unblocks them.**
  - **"A recorded FAIL exits 0."** On 0.1.11 the role composed a full QA1 FAIL verdict and was denied the Write that persists it — visible in the envelope's `permission_denials` as a blocked write to the notes file. It never reached the point of recording, so the exit-code property was never exercised. **Sprint 12's scoped profile is exactly what removes that block**, so demonstrate it here: a role that records a FAIL exits 0. A negative verdict exiting non-zero would make a working gate read as a broken harness.
  - **The interactive path as a real GUI run.** Sprint 11 established no-regression by content — `tasks.json`, `initialPrompt()`, role ids and labels, and the interactive argv all byte-identical — which is sound but is not the same claim. Launch the roles the normal way, via the VS Code task, and confirm seven terminals with each agent announcing its role.
- **A killed launcher must not orphan a billed child.** Sprint 11 found that killing the launcher left the spawned `claude` alive, reparented to PPID 1, still running five minutes later. Standard POSIX and no criterion violated — but the default headless path takes ~170 seconds and the consumer is unattended and will use timeouts, so **every timeout leaks a live billed process.** Establish what actually happens under the scoped profile and record it. If it still orphans, that is a finding for a follow-up sprint, not a failure of this one.
