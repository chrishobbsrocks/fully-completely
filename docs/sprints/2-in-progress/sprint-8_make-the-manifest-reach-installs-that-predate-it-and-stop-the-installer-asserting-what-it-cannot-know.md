...
- **The epic goal, tested directly for the first time.** Install published **0.1.4** into a scratch directory, leave the agent files untouched, upgrade to published 0.1.6, and confirm `.claude/agents/qa1.md` now contains sprint 3's FAIL-demonstration standard and `pipeman.md` contains sprint 4's publish step. This is the thing the entire epic exists to do and it has never once been demonstrated.
