...
5. **Discover, by running them, what each of the six roles needs to hear when nobody will answer. This is the deliverable; the flag is the easy half.** The templates live in this repo and ship to every install — what a headless QA1 needs is a fact about the role, not about one consumer. Launch each role headless, by hand, in sequence, through one throwaway sprint and both gates, and record what was observed.
   - **The message points; it does not summarize.** Do not compose verdicts, failure notes, requirements or phase history into the prompt. All of it is on disk already, written by `sprint_lifecycle.py`, and it is what the interactive roles read. Duplicating it drifts the moment either changes — content that has been through a paraphrase is not the content, which is the failure this project has hit repeatedly this week. A blocked sprint's escalation says *read sprint 4's state*, never restates it.
   - **Expect the fixed scaffold to be most of what you find**, and treat that as a result rather than a shortfall — something close to *you are running unattended; nobody will answer a question; if blocked, record it in the state file and stop rather than asking*, identical across all six. What differs per role is what to point at, not what to explain.
...
- **Req 5: this cannot be cleared by reading.** Confirm the templates came from actual headless runs. **Then read every template for restated state** — a verdict, a note, a requirement, any phase history — and flag each. Pointing is the requirement; summarizing is the defect.
