...
2. **Finding B — decide what `last_shipped_commit` means, and make the answer mechanical.** Two candidate meanings are in play and the code currently implies one while LiveQA checks the other: *the commit whose content was audited and shipped*, or *the commit npm actually stamped as `gitHead`*. They differ whenever bookkeeping lands between audit and publish, which is every release.
   ...
   - **`gitHead` can be absent entirely, and was on 0.1.11.** 0.1.9 and 0.1.10 carry theirs; 0.1.11 does not. The likely cause is publishing from a **linked git worktree** rather than a real clone — which this project now does routinely, so every worktree-shipped release loses npm's own attestation. **LiveQA's criterion 1 depends on that field on every sprint.** Establish whether the worktree theory holds, and give the check a fallback that does not depend on npm recording anything: LiveQA verified 0.1.11 by content instead — registry `dist.shasum` against the downloaded tarball, all 55 published files byte-identical to `git show <commit>:<path>`, and the commit's tree matching the audited tree. That is a stronger proof than the metadata it replaced, and it should be the documented path rather than an improvisation each time.
