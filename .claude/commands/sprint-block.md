...
**Re-filing**: Dev Team runs `/sprint-start <id>` again — the one phase `/sprint-start` accepts besides a genuinely never-started sprint. What happens to the sprint's gate results on re-filing depends on whether anything about the sprint FILE actually changed while it sat blocked (sprint 39, Req 2 — see `/sprint-start`'s own doc for the full two-path breakdown): if a QA1 PASS was already on record and the file is unchanged since, the sprint returns straight to its pre-block phase with both gates intact, no re-audit needed; if the file was edited (the normal case when Master Controller actually repairs a real problem) or no PASS existed yet, it goes back to `dev_build` with both gates cleared, same as before this sprint. Either way, blocking itself is free and non-destructive — nothing here changes based on which path the eventual re-file takes.
