...
3. Define, in the sprint file: objective, requirements, acceptance criteria, dependencies, out-of-scope items, and the `### Human Prerequisites` section (sprint 41) — anything a PERSON must do outside this repo before this sprint's gates can pass (a migration applied to a hosted database, a DNS record, an account, a device at hand), filled in at planning time, not left blank. Something a role can do itself belongs in Requirements instead. An empty section is a normal outcome — write "None," don't pad it with filler or leave it blank.
...
8. **When QA1 routes a predicted operator-side blocker to you** (sprint 41 — QA1's own audit now names this explicitly, as a routed item, rather than burying it as a warning inside verdict `--notes` where it's read after a live-test failure instead of before) — repair the sprint file's `### Human Prerequisites` section with it, the same way you'd repair any other sprint-file gap, and commit that edit yourself before handing it back (step 6 above covers the commit). This is what actually closes the gap: FMC lost two live-test rounds to a real operator-side blocker that QA1's own audit had effectively predicted but that sat unrouted in notes nobody consulted until the live test failed twice.
...
10. Review how the sprint actually went — QA1's audit, LiveQA's live test, any fix loops it took — and fold lessons into the next epic
11. **Before writing a FLAGGED ASSUMPTION, or any claim that a behaviour is unmeasured or unverified, search this repository's own record first** (sprint 47). Four places, specifically, because none of them are covered by just re-reading the current code: sprint files, including CLOSED ones (`docs/sprints/3-done/`, `4-blocked/`, `5-abandoned/`) — a finding doesn't stop existing when its sprint closes; the `docs/` findings documents; CHANGELOG entries; and **annotated git tags** (`git tag -l -n99`, or `git for-each-ref refs/tags --format='%(refname) %(contents)'`), which nothing else in this framework tells anyone to read and which is exactly where 0.1.11's root cause lived, unread, for twenty days. If a prior measurement exists, cite it in the sprint file and state whether it still applies — CLI version, npm behaviour, and dates all move, so "measured once" is not "true forever." If it exists and is now wrong, say so and re-measure deliberately, in the sprint file, rather than flagging the question as never-measured.

    **The cost of skipping this, stated concretely, not as a principle (sprint 47, Req 3a):** sprint 46's own Req 1a called the linked-worktree `gitHead`-dropping behaviour "unverified" when this repository had already measured it twice, twenty days earlier — in the `v0.1.11` git tag annotation and in sprint 13's own sprint file. Writing "unverified" instead of looking cost a real release (0.2.22, published with no `gitHead` at all), a LiveQA FAIL round, and a full fix round, all to re-arrive at a fact already on record. QA1 named the missing discipline afterward: before writing "nobody here knows whether X," look. A rule with a price attached gets followed; an exhortation doesn't.

    **This is planning discipline, not a mechanical gate (sprint 47, Req 3b).** No command in this lifecycle refuses anything over a missing prior-measurement check, and this entry doesn't ask for one — if a mechanical enforcement is ever wanted, that's a finding for a future sprint of its own, decided deliberately, not smuggled in here as a side effect.
