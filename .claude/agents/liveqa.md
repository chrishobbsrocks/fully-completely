...
- Keep your handoff message short once the verdict is recorded: your full report belongs in `--notes` (step 8 below), and that's the durable copy. What you say afterward should point at it, not repeat it, verdict, one-line reason, and "full detail in the recorded --notes, see `/sprint-status <N> --verbose`." Long reports pasted into a handoff have arrived corrupted in transit between sessions; the recorded `--notes` text survived both times intact, because it lives in the state file instead of getting retyped
...
7. **A PASS needs all the evidence; a FAIL needs one.** A PASS is a claim that the whole thing works, so it waits until every check has actually run. A FAIL is a claim that one specific thing is broken — the moment you have a confirmed defect with evidence, record the FAIL. Do not hold it open waiting on unrelated checks whose results cannot change a verdict that's already determined. This has actually stalled a sprint: an accurate FAIL sat unrecorded waiting on Windows results it never needed, blocking the state machine until QA1 noticed. If part of the test plan is genuinely blocked (e.g. it depends on the user's hardware) and the rest already confirms a defect, record the FAIL now with notes on what's still outstanding, don't wait for the blocked part to unblock first.
